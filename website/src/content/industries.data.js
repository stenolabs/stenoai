...
export const COMPLIANCE_BODY =
  "StenographAI runs entirely on your device. Your meeting recordings, transcripts, and summaries never reach our servers — there is no third-party processor handling your meeting data, which addresses a meaningful part of HIPAA, GDPR, and data-residency exposure. (Those frameworks also cover safeguards, agreements, and processes that remain your responsibility — no tool hands you compliance.) StenographAI itself isn't a certified cloud service, because there is no cloud service handling your meetings to certify — and that's the point: the vendor-breach risk that frameworks such as SOC 2 exist to assure against isn't in that path.";
...
//   - `why` must state what StenographAI's architecture actually removes, and must
...
export const government = {
  slug: "government",
  name: "Government",
  metaTitle: "StenographAI for Government — On-Device Meeting Notes, No Cloud",
  metaDescription:
  ...
  eyebrow: "StenographAI for Government",
  h1: "Meeting notes that never leave your perimeter.",
  intro:
    "Briefings, policy discussions, and internal reviews carry information that can't be handed to a cloud vendor. StenographAI records, transcribes, and summarizes entirely on the device — no third-party processor, no data crossing a border, no records leaving the machine they were captured on.",
  ...
  points: [
    ...
    { h: "Works air-gapped", b: "After first-run setup, StenographAI needs no network. It runs on isolated and offline networks where cloud tools simply can't operate." },
    { h: "No bot in the room", b: "StenographAI captures system and microphone audio directly — nothing joins the meeting as a participant." },
    ...
    items: [
      {
        ...
        why: "No tenant setting would have stopped this. The affected departments did nothing wrong — the compromise was inside the provider holding their mail. StenographAI removes that dependency for meeting content: there is no provider account, no key and no server holding your transcripts to be compromised.",
        source: {
      ...
      },
      {
        ...
        why: "Most of those organisations would not have named MOVEit if you had asked them to list their data path. Meeting recordings and transcripts are exactly the kind of data that accumulates in that sort of intermediary. StenographAI's pipeline has no intermediary: capture, transcription and summarisation all happen on the machine.",
        source: {
      ...
      },
      {
        ...
        why: "Whatever the outcome, the exposure is structural: once a notetaker sends audio off the device, consent, retention and model-training become somebody else's policy. StenographAI never sends the audio anywhere, so those questions stay yours to answer.",
        source: {
  ...
  faqs: [
    { q: "Does StenographAI meet data-residency requirements?", a: "Because processing and storage happen on the device and nothing is uploaded, your data never leaves the machine — or the jurisdiction it's in. There is no cloud region to configure and no cross-border transfer to account for." },
    { q: "Can it run on an isolated or air-gapped network?", a: "Yes. Once the models are downloaded during first-run setup, recording, transcription, and summarization all run offline. StenographAI makes no network requests with your meeting content." },
    { q: "How do we verify what it does?", a: "StenographAI is open source (MIT). Your team can audit the code, build it from source, and confirm exactly what it does and doesn't send before deploying." },
  ],
};
...
export const defense = {
  slug: "defense",
  name: "Defence",
  metaTitle: "StenographAI for Defence — Air-Gapped, On-Device Meeting Notes",
  metaDescription:
  ...
  points: [
    { h: "Fully offline", b: "No network dependency after setup. StenographAI runs on air-gapped and disconnected systems where SaaS tools cannot." },
    ...
    items: [
      {
        ...
        why: "Nothing was hacked. A copy was made and sent, and it could not be recalled. StenographAI cannot stop a person emailing a file, and doesn't claim to — but it adds no copies of its own: no vendor-side duplicate, no automatic sync, no export step in the pipeline. Recordings, transcripts and summaries are ordinary local files, held under whatever controls you already run on that machine.",
        source: {
      ...
      },
      {
        ...
        why: "Defence security is bounded by the least-defended system holding the data, and that system frequently belongs to someone else. Every SaaS notetaker adds one. StenographAI adds none: there is no vendor holding your meeting content, so there is no vendor to breach.",
        source: {
  ...
  faqs: [
    { q: "Can StenographAI run in an air-gapped environment?", a: "Yes — that's a primary design target. After the one-time model download, everything runs with no network connection at all." },
    { q: "Is any data ever sent to StenographAI's servers?", a: "No. StenographAI makes no network calls with your meeting content. Recordings, transcripts, and summaries are ordinary files in local storage on your device." },
